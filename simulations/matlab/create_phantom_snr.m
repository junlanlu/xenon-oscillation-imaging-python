%% Create the phantom for SNR plot in paper
...
osc_amp4 = base_value * 0.2;
osc_amp5 = base_value * 0.2;
osc_amp6 = base_value * 0.2;
...
traj_evolve = reshape(z/imSz(1), [npts * nFrames, 1]);
traj_evolve(:, 2) = reshape(x/imSz(2), [npts * nFrames, 1]);
traj_evolve(:, 3) = reshape(y/imSz(3), [npts * nFrames, 1]);
traj_static = traj_evolve;

%% Sample k-space for evolving phantom, simulating RBC dissolved phase
...
data_evolve = reshape(kspIm, [npts * nFrames, 1]);

%% Sample k-space for static phantom, simulating gas phase
kspIm = zeros(npts, nFrames);
for i = 1:nFrames
...
image_osc_gt = imrotate3(image, 90, [1, 0, 0]);
